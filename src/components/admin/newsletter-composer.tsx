        ...
        <div className="grid max-h-[480px] grid-cols-2 gap-3 overflow-y-auto rounded-2xl border border-line bg-cloud p-3 sm:grid-cols-3">
          {products.map((p) => (
            ...
            >
              ...
              {selected.has(p.id) && (
                <span className="absolute right-3 top-3 flex h-6 w-6 items-center justify-center rounded-full bg-pop text-xs font-bold text-cream shadow-soft ring-2 ring-cream">
                  ✓
                </span>
              )}
            </button>
